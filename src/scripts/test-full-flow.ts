...
async function testFullFlow() {
    ...
    try {
        ...
        if (dbResult.Item) {
            ...
            if (
                dbResult.Item.attachments &&
                dbResult.Item.attachments.length > 0
            ) {
                console.log(
                    '✅ Вложения записаны в БД:',
                    dbResult.Item.attachments,
                );
                ...
                try {
                    ...
                    console.log(
                        `✅ Файл успешно найден по новому пути: ${newKey}`,
                    );
                    ...
                    try {
                        ...
                        if (downloadedBuffer.equals(fakeBuffer)) {
                            console.log(
                                '✅ Содержимое файла совпадает с оригиналом',
                            );
                            console.log(
                                `📊 Размер файла: ${downloadedBuffer.length} байт`,
                            );
                        } else {
                            console.error(
                                '❌ Содержимое файла НЕ совпадает с оригиналом!',
                            );
                        }
                    } catch (downloadError) {
                        console.error(
                    ...
                    console.error(
                        `❌ Файл НЕ найден по новому пути: ${newKey}`,
                        s3Error,
                    );
                }
            } else {
