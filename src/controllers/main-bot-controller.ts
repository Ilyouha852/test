...
import { appealRootMachine as supportChatMachine } from '../machines/support-chat.js';
...
class MainBotController {
    ...
    private async processStaffMessage(message: userMessage, connectorName: string): Promise<void> {
        ...
        const event = this.mapSupportChatTextToEvent(message.text);
    ...
    private async loadOrCreateSupportChatMachine(
        userId: string,
        connectorName: string,
        chatId: string,
    ): Promise<any> {
        const snapshotMeta = await stateService.getUserSnapshotWithMeta(
            `${SUPPORT_MACHINE_KEY_PREFIX}:chat:${userId}`,
        );
        if (snapshotMeta?.snapshot) {
            console.log(
                `📦 [SupportChat] Restoring machine (state: ${snapshotMeta.currentState})`,
            );
            return this.restoreActor(supportChatMachine, snapshotMeta.snapshot);
        }
        console.log('🆕 [SupportChat] Creating new support chat machine');
        const actor = createActor(supportChatMachine, {
            input: { userId, connectorName, chatId },
        });
        actor.start();
        return actor;
    ...
    private mapSupportChatTextToEvent(text: string): any {
        const normalized = text.trim().toUpperCase();
        if (normalized.startsWith('SELECT_APPEAL')) {
            const parts = text.trim().split(/\s+/);
            return parts[1]
                ? { type: 'SELECT_APPEAL', appealId: parts[1] }
                : { type: 'HELP' };
        }
        const map: Record<string, any> = {
            ...
            ПОДТВЕРДИТЬ: { type: 'CONFIRM' },
            SEND: { type: 'SEND' },
            ОТПРАВИТЬ: { type: 'SEND' },
            SOLVE: { type: 'SOLVE' },
            РЕШИТЬ: { type: 'SOLVE' },
            CHANGE_ATTACH_STATUS: { type: 'CHANGE_ATTACH_STATUS' },
            CHANGE_STATUS_APPEAL: { type: 'CHANGE_STATUS_APPEAL' },
        };
        return map[normalized] ?? { type: 'HELP' };
    }
