...
async function runRepairBotTests() {
  ...
  try {
    ...
    const startResponse = await axios.post(`${API_BASE}/sessions/${userId}/start`);
    ...
    const problemResponse = await axios.post(`${API_BASE}/sessions/${userId}/event`, {
      type: 'DESCRIBE_PROBLEM',
      problem: 'Дверь скрипит при открывании'
    });
    console.log('   ✅ Problem processed:', problemResponse.data.currentState);
    
    await new Promise(resolve => setTimeout(resolve, 300));
    
    // Тест 3: Получение состояния
    console.log('3. 📊 Testing state retrieval...');
    const stateResponse = await axios.get(`${API_BASE}/sessions/${userId}/state`);
    ...
    const confirmResponse = await axios.post(`${API_BASE}/sessions/${userId}/event`, {
      type: 'CONFIRM'
    });
    console.log('   ✅ Solution confirmed:', confirmResponse.data.currentState);
    
    await new Promise(resolve => setTimeout(resolve, 300));
    
    // Тест 5: Финальное состояние
    console.log('5. 📋 Testing final state...');
    const finalState = await axios.get(`${API_BASE}/sessions/${userId}/state`);
    console.log('   ✅ Final state:', finalState.data.currentState);
    console.log('   ✅ Total history entries:', finalState.data.history.length);
    
    // Тест 6: Завершение сессии
    console.log('6. 🏁 Testing session end...');
    const endResponse = await axios.delete(`${API_BASE}/sessions/${userId}`);
    console.log('   ✅ Session ended:', endResponse.data.message);
    
    console.log('\n🎉 All tests passed successfully!');
    console.log('📈 Workflow: idle → awaitingProblem → solution → success → idle');
